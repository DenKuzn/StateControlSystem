...
#include "BoxStrategy/GameAI/AbilitySystem/Abilities/Settings/Warrior/UGAS_Warrior_Attack.h"
...
{
	UBoxStrategyAbilitySystemComponent* UnitAbilityComponent = GetBoxStrategyAbilitySystemComponent();
	{
#if GAME_DEBUG_BUILDS
	...
	// Собираем приказ для атаки, чтобы можно было повторно запустить атаку.
	{
		...
		AttackSettings->SetAbilityTag( FBoxStrategyNativeGameplayTags::Get().BoxStrategy_Ability_Unit_Warrior_Other_StandardAttack );
	...
	// Собираем приказ для движения, пока не будет виден противник, чтобы можно было в него стрелять напрямую.
	{
		...
		MoveToSightSettings->SetAbilityTag( FBoxStrategyNativeGameplayTags::Get().BoxStrategy_Ability_Unit_Misc_MoveToSight );
